...
def _run_ingest(key, rfp_id, filename, gcs_path):
    ...
    try:
        ...
        # Mark text ingest complete immediately so LWC polling turns green in seconds.
        if transcribed:
        ...
        # Cloud Run has --no-cpu-throttling enabled, so this thread runs at 100% CPU.
        if filename.lower().endswith(".pdf") and config.GCS_BUCKET:
            def _archive_pages():
                try:
                    logger.info("Starting background page image archiving for %s", filename)
                    ...
                    logger.info("Finished background page image archiving for %s", filename)
                except Exception:
